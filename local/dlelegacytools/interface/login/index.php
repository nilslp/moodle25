...
if ($frm and isset($frm->username)) {                             // Login WITH cookies
    ...
    /// Prepare redirection
    	$cf_redirect=get_config('local/dlelegacytools','loginredirect');
        if (user_not_fully_set_up($USER) && $cf_redirect!== 'force') {
            $urltogo = $CFG->wwwroot.'/user/edit.php';
            // We don't delete $SESSION->wantsurl yet, so we get there later

        } else if ((isset($SESSION->wantsurl) and (strpos($SESSION->wantsurl, $CFG->wwwroot) === 0 or strpos($SESSION->wantsurl, str_replace('http://', 'https://', $CFG->wwwroot)) === 0)) && $cf_redirect!== 'force' ){
            $urltogo = $SESSION->wantsurl;    /// Because it's an address in this site
            unset($SESSION->wantsurl);
			
		}else if($cf_redirect === 'force'){
			$urltogo = $CFG->wwwroot . get_config('local/dlelegacytools','loginredirect_target');
			unset($SESSION->wantsurl);
