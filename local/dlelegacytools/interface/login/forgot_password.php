...
/**
 * Implements a custom forgotten password page 
 *
 * @package    local
...
$PAGE->set_url('/local/dlelegacytools/interface/login/forgot_password.php');
...
$mform = new dlt_login_forgot_password_form();
    ...
    if ($user and !empty($user->confirmed)) {
        ...
        if ($userauth->can_reset_password() and is_enabled_auth($user->auth)
          and has_capability('moodle/user:changeownpassword', $systemcontext, $user->id)) {
            ...
            // custom email function DMPH
            if (!dlt_send_password_change_confirmation_email($user)) {
...
echo $OUTPUT->header();
echo $OUTPUT->box(get_string('passwordforgotteninstructions','local_dlelegacytools'), 'generalbox boxwidthnormal boxaligncenter');
$mform->display();
