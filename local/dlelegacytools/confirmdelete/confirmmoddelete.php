...
/*
 * NOTE: This file was copied and ammended to provide extra detail on a delete
 * to provide the user with more information to confirm the deletion
 */
require("../../../config.php");
...
$is_course     = optional_param('course', 0, PARAM_INT);
...
if (!empty($add)) {
    ...
    if (!$confirm or !confirm_sesskey()) {
        if ($cm->modname == 'facetoface') {
            $systemcontext = get_context_instance(CONTEXT_SYSTEM);
            
            // Deleting a facetoface module requires elevated capabilities
            require_capability('mod/facetoface:deleteinstance', $systemcontext);
        }
        ...
        if ($is_course != 0 && isset($course) && ($course->id > 0)) {
        ...
        echo $OUTPUT->confirm($strdeletecheckfull . $table_string, $formcontinue, $formcancel);
