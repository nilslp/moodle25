 ...
 */
function callback_lpdefault_uses_sections() {
    return true;
}
 ...
 * @return bool Returns true
 */
function callback_lpdefault_load_content(&$navigation, $course, $coursenode) {
    return $navigation->load_generic_course_sections($course, $coursenode, 'lpdefault');
}
 ...
 * @return string
 */
function callback_lpdefault_definition() {
    return get_string('lpdefault');
}
 ...
 * @return string
 */
function callback_lpdefault_request_key() {
    return 'lpdefault';
}

function callback_lpdefault_get_section_name($course, $section) {
    // We can't add a node without any text
    if (!empty($section->name)) {
        return format_string($section->name, true, array('context' => get_context_instance(CONTEXT_COURSE, $course->id)));
    } else if ($section->section == 0) {
        return get_string('section0name', 'format_lpdefault');
    } else {
 ...
 * @return stdClass
 */
function callback_lpdefault_ajax_support() {
 ...
 * @return moodle_url
 */
function callback_lpdefault_get_section_url($courseid, $sectionnum) {
