...
$url = new moodle_url('/admin/tool/elementlibrary2/pagelayouts.php', array('layout' => $layout));
$PAGE->set_url($url);
...
admin_externalpage_setup('toolelementlibrary2');

echo $OUTPUT->header();

if ($layout) {
    echo html_writer::link(new moodle_url('/admin/tool/elementlibrary2/pagelayouts.php'), '&laquo; Back to layouts');
} else {
    echo html_writer::link(new moodle_url('/admin/tool/elementlibrary2/'), '&laquo; Back to index');
}
...
if ($layout) {
    ...
    foreach ($layouts as $name => $info) {
        $url = new moodle_url('/admin/tool/elementlibrary2/pagelayouts.php', array('layout' => $name));
        $text = $info['name'];
        if ($name != 'popup') {
