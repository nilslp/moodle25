...
function update_tag_grouping($groupid) {
    ...
    $tagid = $DB->get_field('report_builder_group', 'assignvalue', 'assigntype',
        'tag', 'id', $groupid);
    if(!$tagid) {
        // need a tag
        return false;
    ...
    $tag = $DB->get_record('tag', 'id', $tagid, 'tagtype', 'official');
    if(!$tag) {
    ...
    $tagged_activities = $DB->get_records_sql($sql);
    ...
    foreach($tagged_activities as $tagged) {
        ...
        if(!$track) {
            ...
            $status = $status && $DB->insert_record('report_builder_group_assign',
                $todb);
        }
    }
    ...
    foreach($assigned_activities as $assigned) {
        if(!array_key_exists($assigned->itemid, $tagged_activities)) {
            $badid = $assigned->id;
            $status = $status && $DB->delete_records('report_builder_group_assign',
                'id', $badid);
        }
    }
    ...
    if($status) {
    	...
    	$transaction->rollback("Error in update_tag_grouping");
        return false;
    }
}
