...
class rb_role_access extends rb_base_access {
    ...
    function access_restriction($reportid) {
        ...
        if($contextsetting == 'any') {
            ...
            $userroles = $DB->get_records_sql_menu("SELECT DISTINCT roleid, 1
    ...
    function form_template(&$mform, $reportid) {
        ...
        $mform->addElement('header', 'accessbyroles', get_string('accessbyrole', 'block_lp_reportbuilder'));

        //TODO replace with checkbox once there is more than one option
        $mform->addElement('hidden', 'role_enable', 1);

        if ($roles = $DB->get_records('role', array(), 'sortorder')) {
            $contextoptions = array('site' => get_string('systemcontext','block_lp_reportbuilder'), 'any' => get_string('anycontext','block_lp_reportbuilder'));

            // set context for role-based access
            $mform->addElement('select','role_context', get_string('context', 'block_lp_reportbuilder'), $contextoptions);
            $mform->setDefault('role_context', $context);
            $mform->disabledIf('role_context', 'accessenabled', 'eq', 0);
           // $mform->setHelpButton('role_context', array('reportbuildercontext',get_string('context','block_lp_reportbuilder'),'block_lp_reportbuilder'));
            ...
            $mform->addGroup($rolesgroup, 'roles', get_string('roleswithaccess','block_lp_reportbuilder'), '<br />', false);
            $mform->disabledIf('roles', 'accessenabled', 'eq', 0);
            //$mform->setHelpButton('roles', array('reportbuilderrolesaccess',get_string('roleswithaccess','block_lp_reportbuilder'),'local_reportbuilder'));
        } else {
            $mform->addElement('html', '<p>'.get_string('error:norolesfound','block_lp_reportbuilder').'</p>');
        }
