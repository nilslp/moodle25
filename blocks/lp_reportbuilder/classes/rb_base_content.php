...
class rb_current_pos_content extends rb_base_content {
    ...
    function sql_restriction($field, $reportid) {
        ...
        $posid = $DB->get_field('pos_assignment', 'positionid', 'userid', $userid,
            'type', 1);
    ...
    function text_restriction($title, $reportid) {
        ...
        $posid = $DB->get_field('pos_assignment', 'positionid',
            'userid', $userid, 'type', 1);
        $posname = $DB->get_field('pos','fullname','id', $posid);
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'current_pos_header',
            get_string('showbyx','block_lp_reportbuilder', lowerfirst($title)));
        $mform->addElement('checkbox', 'current_pos_enable', '',
            get_string('currentposenable','block_lp_reportbuilder'));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'current_pos_recursive',
            '', get_string('showrecordsinposandbelow', 'block_lp_reportbuilder'), 1);
        $radiogroup[] =& $mform->createElement('radio', 'current_pos_recursive',
            '', get_string('showrecordsinpos', 'block_lp_reportbuilder'), 0);
        $radiogroup[] =& $mform->createElement('radio', 'current_pos_recursive',
            '', get_string('showrecordsbelowposonly', 'block_lp_reportbuilder'), 2);
        $mform->addGroup($radiogroup, 'current_pos_recursive_group',
            get_string('includechildpos','block_lp_reportbuilder'), '<br />', false);
 ...
 //           get_string('showbycurrentpos', 'block_lp_reportbuilder'), 'block_lp_reportbuilder'));
    }
...
class rb_current_org_content extends rb_base_content {
    ...
    function sql_restriction($field, $reportid) {
        ...
        $orgid = $DB->get_field('pos_assignment', 'organisationid', 'userid', $userid,
            'type', 1);
        // no results if they don't have one
        if(empty($orgid)) {
            // using 1=0 instead of FALSE for MSSQL support
...
class rb_completed_org_content extends rb_base_content {
    ...
    function text_restriction($title, $reportid) {
        ...
        $orgid = $DB->get_field('pos_assignment', 'organisationid',
            'userid', $userid, 'type', 1);
        if(empty($orgid)) {
            return $title . ' ' . get_string('is','local_reportbuilder') . ' "UNASSIGNED"';
        }
        $orgname = $DB->get_field('org','fullname','id', $orgid);
...
class rb_user_content extends rb_base_content {
    ...
    function text_restriction($title, $reportid) {
        ...
        switch ($settings['who']) {
        case 'own':
            return $title . ' ' . get_string('is','block_lp_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'reports':
            return $title . ' ' . get_string('reportsto','block_lp_reportbuilder') . ' "' .
                fullname($user) . '"';
        case 'ownandreports':
            return $title . ' ' . get_string('is','block_lp_reportbuilder') . ' "' .
                fullname($user) . '"' . get_string('or','block_lp_reportbuilder') .
                get_string('reportsto','block_lp_reportbuilder') . ' "' . fullname($user) . '"';
        default:
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'user_header', get_string('showbyx',
            'block_lp_reportbuilder', lowerfirst($title)));
        $mform->addElement('checkbox', 'user_enable', '',
            get_string('showbasedonx', 'block_lp_reportbuilder', lowerfirst($title)));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'user_who', '',
            get_string('userownrecords', 'block_lp_reportbuilder'), 'own');
        $radiogroup[] =& $mform->createElement('radio', 'user_who', '',
            get_string('userstaffrecords', 'block_lp_reportbuilder'), 'reports');
        $radiogroup[] =& $mform->createElement('radio', 'user_who', '',
            get_string('both', 'block_lp_reportbuilder'), 'ownandreports');
        $mform->addGroup($radiogroup, 'user_who_group',
            get_string('includeuserrecords', 'block_lp_reportbuilder'), '<br />', false);
...
//        $mform->setHelpButton('user_header', array('reportbuilderuser',
 //           get_string('showbyuser', 'block_lp_reportbuilder'), 'block_lp_reportbuilder'));
    }
...
class rb_date_content extends rb_base_content {
    ...
    function sql_restriction($field, $reportid) {
        ...
        case 'last7days':
            return '( (' . $field . ' < ' . $now . ' AND ' . $field . ' > ' .
                ($now - 60*60*24*7) . ')' . $includenulls . ')';
        case 'last30days':
            return '( (' . $field . ' < ' . $now . ' AND ' . $field . ' > ' .
                ($now - 60*60*24*30) . ')' . $includenulls . ')';
        case 'last60days':
            return '( (' . $field . ' < ' . $now . ' AND ' . $field . ' > ' .
                ($now - 60*60*24*60) . ')' . $includenulls . ')';
        case 'last90days':
            return '( (' . $field . ' < ' . $now . ' AND ' . $field . ' > ' .
                ($now - 60*60*24*90) . ')' . $includenulls . ')';            
        case 'next30days':
            return '( (' . $field . ' > ' . $now . ' AND ' . $field . ' < ' .
                ($now + 60*60*24*30) . ')' . $includenulls . ')';
        case 'currentfinancial':
            ...
            if(date('n', $now) >= 4 && date('j', $now) >= 6) { // date is on or after 1st April
                $start = mktime(0, 0, 0, 4, 6, $required_year);
                $end = mktime(0, 0, 0, 4, 6, $year_after);
            } else {
                $start = mktime(0, 0, 0, 4, 6, $year_before);
                $end = mktime(0, 0, 0, 4, 6, $required_year);
            }
        ...
        case 'lastfinancial':
            ...
            $year_before = $required_year - 1;
            $year_after = $required_year + 1;
            // TODO -- BQ -- Is this a Totara bug? (hard-coded financial year)
            if(date('n', $now) >= 4 && date('j', $now) >= 6) { // date is on or after 1st April
                $start = mktime(0, 0, 0, 4, 6, $required_year);
                $end = mktime(0, 0, 0, 4, 6, $year_after);
            } else {
    ...
    function text_restriction($title, $reportid) {
        ...
        switch ($settings['when']) {
        case 'past':
            return $title . ' ' . get_string('occurredbefore', 'block_lp_reportbuilder') . ' ' .
                userdate(time(), '%c'). $includenulls;
        case 'future':
            return $title . ' ' . get_string('occurredafter', 'block_lp_reportbuilder') . ' ' .
                userdate(time(), '%c'). $includenulls;
        case 'last7days':
            return $title . ' ' . get_string('occurredafter','block_lp_reportbuilder') . ' ' .
                userdate(time() - 60*60*24*7, '%c') . get_string('and','block_lp_reportbuilder') .
                get_string('occurredbefore','block_lp_reportbuilder') . userdate(time(),'%c') .
                $includenulls;
        case 'last30days':
            return $title . ' ' . get_string('occurredafter','block_lp_reportbuilder') . ' ' .
                userdate(time() - 60*60*24*30, '%c') . get_string('and','block_lp_reportbuilder') .
                get_string('occurredbefore','block_lp_reportbuilder') . userdate(time(),'%c') .
                $includenulls;
        case 'last60days':
            return $title . ' ' . get_string('occurredafter','block_lp_reportbuilder') . ' ' .
                userdate(time() - 60*60*24*60, '%c') . get_string('and','block_lp_reportbuilder') .
                get_string('occurredbefore','block_lp_reportbuilder') . userdate(time(),'%c') .
                $includenulls;
        case 'last90days':
            return $title . ' ' . get_string('occurredafter','block_lp_reportbuilder') . ' ' .
                userdate(time() - 60*60*24*90, '%c') . get_string('and','block_lp_reportbuilder') .
                get_string('occurredbefore','block_lp_reportbuilder') . userdate(time(),'%c') .
                $includenulls;
        case 'next30days':
            return $title . ' ' . get_string('occurredafter','block_lp_reportbuilder') . ' ' .
                userdate(time(), '%c') . get_string('and', 'block_lp_reportbuilder') .
                get_string('occurredbefore', 'local_reportbuilder') .
                userdate(time() + 60*60*24*30,'%c') . $includenulls;
        case 'currentfinancial':
            return $title . ' ' . get_string('occurredthisfinancialyear','block_lp_reportbuilder') .
                $includenulls;
        case 'lastfinancial':
            return $title . ' ' . get_string('occurredprevfinancialyear','block_lp_reportbuilder') .
                $includenulls;
        default:
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'date_header', get_string('showbyx',
            'block_lp_reportbuilder', $title));
        $mform->addElement('checkbox', 'date_enable', '',
            get_string('showbasedonx', 'block_lp_reportbuilder',
            $title));
        ...
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('thepast', 'block_lp_reportbuilder'), 'past');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('thefuture', 'block_lp_reportbuilder'), 'future');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('last7days', 'block_lp_reportbuilder'), 'last7days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('last30days', 'block_lp_reportbuilder'), 'last30days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('last60days', 'block_lp_reportbuilder'), 'last60days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('last90days', 'block_lp_reportbuilder'), 'last90days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('next30days', 'block_lp_reportbuilder'), 'next30days');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('currentfinancial', 'block_lp_reportbuilder'), 'currentfinancial');
        $radiogroup[] =& $mform->createElement('radio', 'date_when', '',
            get_string('lastfinancial', 'block_lp_reportbuilder'), 'lastfinancial');
        $mform->addGroup($radiogroup, 'date_when_group',
            get_string('includerecordsfrom', 'block_lp_reportbuilder'), '<br />', false);
 ...
 //           get_string('showbydate', 'block_lp_reportbuilder'), 'block_lp_reportbuilder'));

        $mform->addElement('checkbox', 'date_incnulls',
            get_string('includeemptydates', 'block_lp_reportbuilder'));
...
/*
 * Extended date content class
 *  - ran into form validation issues with multiple date content options
 *    on a report, so I included this new class to solve the problem. This also
 *    preserves previous date_content that has been added to reports. DMPH
 *
 * Pass in an integer that contains a unix timestamp
 */
class rb_ex_date_content extends rb_base_content {
    /**
     * Generate the SQL to apply this content restriction
     *
     * @param string $field SQL field to apply the restriction against
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'date_header', get_string('showbyx',
            'block_lp_reportbuilder', $title));
        $mform->addElement('checkbox', "{$type}_date_enable", '',
            get_string('showbasedonx', 'block_lp_reportbuilder',
            $title));
        $mform->setDefault("{$type}_date_enable", $enable);
        $mform->disabledIf("{$type}_date_enable", 'contentenabled', 'eq', 0);
        $radiogroup = array();
        $radiogroup[] =& $mform->createElement('radio', "{$type}_date_when", '',
            get_string('thepast', 'block_lp_reportbuilder'), 'past');
        $radiogroup[] =& $mform->createElement('radio',  "{$type}_date_when", '',
            get_string('thefuture', 'block_lp_reportbuilder'), 'future');
        $radiogroup[] =& $mform->createElement('radio',  "{$type}_date_when", '',
        ...
        $radiogroup[] =& $mform->createElement('radio',  "{$type}_date_when", '',
            get_string('lastfinancial', 'block_lp_reportbuilder'), 'lastfinancial');
        $mform->addGroup($radiogroup,  "{$type}_date_when_group",
            get_string('includerecordsfrom', 'block_lp_reportbuilder'), '<br />', false);
        $mform->setDefault( "{$type}_date_when", $when);
        $mform->disabledIf( "{$type}_date_when_group", 'contentenabled', 'eq', 0);
        $mform->disabledIf( "{$type}_date_when_group", "{$type}_date_enable", 'notchecked');
 //       $mform->setHelpButton('date_header',
 //           array('reportbuilderdate',
 //           get_string('showbydate', 'block_lp_reportbuilder'), 'block_lp_reportbuilder'));

        $mform->addElement('checkbox', "{$type}_date_incnulls",
            get_string('includeemptydates', 'block_lp_reportbuilder'));
        $mform->setDefault("{$type}_date_incnulls", $incnulls);
        $mform->disabledIf("{$type}_date_incnulls", "{$type}_date_enable", 'notchecked');
        $mform->disabledIf("{$type}_date_incnulls", 'contentenabled', 'eq', 0);
    }
    ...
    function form_process($reportid, $fromform) {
        ...
        // enable checkbox option
        $field = "{$type}_date_enable";
        $enable = (isset($fromform->{$field}) &&
            $fromform->{$field}) ? 1 : 0;
        $status = $status && reportbuilder::update_setting($reportid, $type,
            'enable', $enable);

        // when radio option
        $field = "{$type}_date_when";
        $when = isset($fromform->{$field}) ?
            $fromform->{$field} : 0;
        $status = $status && reportbuilder::update_setting($reportid, $type,
            'when', $when);

        // include nulls checkbox option
        $field = "{$type}_date_incnulls";
        $incnulls = (isset($fromform->{$field}) &&
            $fromform->{$field}) ? 1 : 0;
        $status = $status && reportbuilder::update_setting($reportid, $type,
            'incnulls', $incnulls);
...
class rb_course_tag_content extends rb_base_content {
    ...
    function sql_restriction($field, $reportid) {
        ...
        $tags = $DB->get_records('tag', 'tagtype', 'official', 'name');
        if($tags) {
            foreach($tags as $tag) {
    ...
    function text_restriction($title, $reportid) {
        ...
        if(count($include_text) > 0) {
            $includestr = $title . ' ' . get_string('istaggedwith', 'block_lp_reportbuilder') .
                ' ' . implode(get_string($include_logic, 'block_lp_reportbuilder'), $include_text);
        } else {
            $includestr = '';
        }
        if(count($exclude_text) > 0) {
            $excludestr = $title . ' ' . get_string('isnttaggedwith', 'block_lp_reportbuilder') .
                ' ' . implode(get_string($exclude_logic, 'block_lp_reportbuilder'), $exclude_text);
        } else {
            $excludestr = '';
        }

        if($includestr && $excludestr) {
            return $includestr . get_string('and','block_lp_reportbuilder') . $excludestr;
        } else if ($includestr) {
            return $includestr;
        } else if ($excludestr) {
            return $excludestr;
    ...
    function form_template(&$mform, $reportid, $title) {
        ...
        $mform->addElement('header', 'course_tag_header',
            get_string('showbycoursetag','block_lp_reportbuilder'));
 //       $mform->setHelpButton('course_tag_header',
 //           array('reportbuildercoursetag',
 //           get_string('showbycoursetag', 'block_lp_reportbuilder'), 'block_lp_reportbuilder'));

        $mform->addElement('checkbox', 'course_tag_enable', '',
            get_string('coursetagenable','block_lp_reportbuilder'));
        ...
        $tags = $DB->get_records('tag', 'tagtype', 'official','name');
        if($tags) {
            $opts = array(1 => get_string('anyofthefollowing','block_lp_reportbuilder'),
                          0 => get_string('allofthefollowing','block_lp_reportbuilder'));
            $mform->addElement('select','course_tag_include_logic', get_string('includecoursetags','block_lp_reportbuilder'), $opts);
        ...
        if($tags) {
            $opts = array(1 => get_string('anyofthefollowing','block_lp_reportbuilder'),
                          0 => get_string('allofthefollowing','block_lp_reportbuilder'));
            $mform->addElement('select','course_tag_exclude_logic', get_string('excludecoursetags','block_lp_reportbuilder'), $opts);
    ...
    function form_process($reportid, $fromform) {
        ...
        // tag settings
        $tags = $DB->get_records('tag', 'tagtype', 'official');
        if($tags) {
...
include_once($CFG->dirroot . '/blocks/lp_reportbuilder/classes/rb_trainer_content.php');
