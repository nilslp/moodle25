...
$PAGE->set_title(get_string('scheduledreportsforuser','block_lp_reportbuilder'));
$PAGE->set_heading($SITE->fullname);
$home_url = $CFG->wwwroot;
$PAGE->navbar->add(get_string('scheduledreportsforuser','block_lp_reportbuilder'));
echo $OUTPUT->header();
...
if ($can_view && reportbuilder_get_reports()){
    ...
    echo '<h2>' . get_string('scheduledreportsforuser', 'block_lp_reportbuilder') . '</h2>';
...
function print_scheduled_reports($return=false) {
    ...
    $mform = new scheduled_reports_add_on_behalf_form($CFG->wwwroot . '/blocks/lp_reportbuilder/scheduledonbehalf.php', array());
    $mform->display();

    $sql = "SELECT rbs.*, rb.fullname, u.firstname, u.lastname
            FROM {$CFG->prefix}report_builder_schedule rbs
            JOIN {$CFG->prefix}report_builder rb ON rbs.reportid = rb.id
            JOIN {$CFG->prefix}user u ON u.id = rbs.userid
            WHERE rbs.createdby = {$USER->id} AND rbs.onbehalfof = 1
            ORDER BY rbs.timecreated ASC";

    if ($scheduledreports = $DB->get_records_sql($sql)){
        $columns[] = 'recipient';
        $headers[] = get_string('recipient', 'block_lp_reportbuilder');
        $columns[] = 'reportname';
        ...
        foreach ($scheduledreports as $sched) {
            ...
            $tablerow = array();
            $tablerow[] = $sched->firstname . ' ' . $sched->lastname;
            ...
            */
            ...
            $sched_opts.= html_writer::link($CFG->wwwroot.'/blocks/lp_reportbuilder/scheduledonbehalf.php?id='.$sched->id,$icon_edit,array('title'=>get_string('edit')));
        ...
        echo get_string('noscheduledreportsonbehalfof', 'block_lp_reportbuilder') . '<br /><br />';
    }
}
