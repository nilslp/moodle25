<?php //$Id$
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/text.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/textarea.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/number.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/simpleselect.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/select.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/selecttwo.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/selectin.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/date.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/hierarchy.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/multicheck.php');
require_once($CFG->dirroot.'/blocks/lp_reportbuilder/filters/filter_forms.php');
...
class filtering {
    ...
    function filtering($report=null, $baseurl=null, $extraparams=null) {
        ...
        if ($adddata = $this->_addform->get_data(false)) {
            ...
            if(isset($adddata->clearfilter)) {
                    ...
                    if ($sid !== 0) {
                        // Clear was pressed while the report was being viewed
                        // from a saved search so redirect to the original report URL    
    ...
    /**
     * Tells us that filtering is a go 
     */
    function is_form_submitted() {
        return ($this->_addform && $this->_addform->is_submitted());
    }
...
function search_parse_keywords($query) {
    ...
    foreach($split_quoted as $item) {
        ...
        if(preg_match('/^(["\'])(.*)\\1$/', trim($item), $matches)) {
            ...
            // BQ: addslashes_recursive has been deprecated
            /*
            $keyword = addslashes_recursive(preg_split('/\s/', $item, 0,
                PREG_SPLIT_NO_EMPTY));
                */
            $keyword = preg_split('/\s/', $item, 0, PREG_SPLIT_NO_EMPTY);
        	//print_r($keyword); die();
...
function search_get_keyword_where_clause($field, $keywords, $negate=false) {
    ...
    foreach($keywords as $keyword) {
        $queries[] = $field . ' ' . $not . 'LIKE' . " '%" . $keyword . "%'";
    }
