 ...
 */
class filter_selecttwo extends filter_type {
    /**
     ...
     */
    function filter_selecttwo($filter, $sessionname, $selectoptions, $default=null, $attributes=null) {
    ...
    function get_sql_filter($data) {
        ...
        return '(' . $alias . '.' . $query . $token . '(' . $value . '))';
    }
...
/*    function get_sql_filter($data) {
        ...
        if ($operator == 2) {
            ...
            $string = '('.$query.$token."'$value' OR ".$query.' IS NULL)';
        } 
        else {
            $string = $query.$token."'$value'";
        }
