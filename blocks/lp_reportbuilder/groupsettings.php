 ...
 */
    ...
    $group = $DB->get_record('report_builder_group', 'id', $id);
    $tag = $DB->get_record('tag', 'id', $group->assignvalue);
    ...
    $baseitem = $DB->get_record_sql($sql);

    // find out which reports use this group
    $reports = $DB->get_records_select('report_builder', 'source ' . $DB->sql_ilike() .
        "'%_grp_" . $id . "'");
