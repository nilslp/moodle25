M.theme_foundation.toggle_course_list = function(Y) {
    ...
    if (page_course_index) {
        ...
        toggle = Y.Node.create('<div class="toggle-view">'+M.util.get_string('course-list-show-az','theme_foundation')+'</div>');
        ...
        if (collapseable) {
            if (collapseable === 1) {
                 az_list_collapse_button = Y.Node.create('<div class="az-accordian expand"><span>'+M.util.get_string('course-list-expand-all','theme_foundation')+'</span></div>');
            }
            else if (collapseable === 2) {
                 az_list_collapse_button = Y.Node.create('<div class="az-accordian"><span>'+M.util.get_string('course-list-collapse-all','theme_foundation')+'</span></div>');
            }
        ...
        page_course_index.all('.toggle-view').on("click", function (e) {
            if(cat_list.hasClass('hide')) {
                ...
                toggle.setContent(M.util.get_string('course-list-show-az','theme_foundation'));
            }
            else {
                toggle.setContent(M.util.get_string('course-list-show-cat','theme_foundation'));
        ...
        if (collapseable === 0) {
            ...
            page_course_index.one('.region-content .box.categorybox .course_category_tree').prepend('<div class="cat-accordian expand"><span>'+M.util.get_string('course-list-expand-all','theme_foundation')+'</span></div>');
            all_cats.addClass('collapsed');
        }
        else if (collapseable === 2) {
            page_course_index.one('.region-content .box.categorybox .course_category_tree').prepend('<div class="cat-accordian"><span>'+M.util.get_string('course-list-collapse-all','theme_foundation')+'</span></div>');
            all_cats.removeClass('collapsed');
        }
        ...
        cat_accordian && cat_accordian.on('click',
            function() {
                ...
                if (this.hasClass('expand')) {
                    ...
                    this.one('span').set('text',M.util.get_string('course-list-collapse-all','theme_foundation'));
                    all_cats.removeClass('collapsed');
                }
                else {
                    this.addClass('expand');
                    this.one('span').set('text',M.util.get_string('course-list-expand-all','theme_foundation'));
                    all_cats.addClass('collapsed');
                }
             }
        );
